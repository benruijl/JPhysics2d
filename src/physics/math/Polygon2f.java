...
public class Polygon2f {
    ...
    public CollisionData circleCollisionData(final Circle circle,
            final Vector2f velocity) {
        final float time = circleCollision(circle, velocity);

        if (time < 0) {
            return new CollisionData(false, 0, null, null);
        }
        ...
        return new CollisionData(true, time, normal, penetration);
    }
}
